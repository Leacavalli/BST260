...
```{r, eval=FALSE}
polls1 <- polls %>%
  mutate(p_hat = (1+d_hat)/2,
         moe = 2*1.96*sqrt(p_hat*(1-p_hat)/samplesize),
         lower = d_hat - moe,
         upper = d_hat + moe) %>%
  select(pollster, enddate, d_hat, lower, upper)
head(polls1)

polls2 <- polls1 %>% mutate(hit = lower<=0.021 & upper>=0.021)
```
...
```{r, eval=FALSE}
polls3 <- polls1 %>% mutate(error = d_hat-0.021)

ggplot(polls3, aes(x=error, y=pollster))+
  geom_point()
...
ggplot(polls4, aes(x=error, y=pollster))+
  geom_point()
