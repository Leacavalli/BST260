...
# correct answer is D. Rafa does not like pie charts 
...
```{r, eval=FALSE}
state <- dat$state
#view top 5 rows. Here we confirm that the object state is in alphabetical order
head(state)
...
#confirm that the levels of the 'state' object are re-ordered but the vector itself has not been re-ordered
head(state)
levels(state)
```
...
#I created a new 'd' table instead of redefining the dat table above to see the changes. 
d <- dat %>%
  mutate(rate = count/population*10000*52/weeks_reporting,
        state = reorder(state,rate))
...
# pretty sure the correct answer is c but it may be b?? UPDATE. Cannot be b bc the plot would should standard deviations...not standard errors which are population parameters
...
```{r}
#make boxplot. Not sure what is meant by showing all the points
murders |> mutate(region = reorder(region, rate, FUN = median)) |>ggplot(aes(x=region,y=rate, fill=region)) + geom_boxplot() + geom_jitter()
