...
*Approximately 25% of the states are in the North Central region*  
  
  
  2\. Which of the following is true:
  
a. The graph above is a histogram. *No*
b. The graph above shows only four numbers with a bar plot. *yes*
c. Categories are not numbers, so it does not make sense to graph the distribution.*no*
d. The colors, not the height of the bars, describe the distribution. *No*
...
#using this code we see the right answer is 1
murders |> mutate(murder_rate = total/population * 10^5) |> arrange(-murder_rate)
```
...
Knowing that there are 51 states (counting DC) and based on this plot, how many states have murder rates larger than 10 per 100,000 people?
  ...
  a. 1 *maddy -- I think this one maybe?* *Paola: I agree this is the correct answer)*
b. 5
c. 10
d. 50 *Yes*
...
c. All the states have murder rates above 2 per 100,000. *Yes* *I don't think so (eva)* *Paola: checked using code and this is not correct*
d. With the exception of 4 states, the murder rates are below 5 per 100,000. *maddy  -- maybe this one; eva agrees* *paola; agreed*
...
Based on this plot, how many males are between 63.5 and 65.5? *maddy -- i think in slack they said this is ~47; it looked like 50ish to me*

#answer choices below are incorrect
  a. 10
b. 24
c. 34
d. 100 
...
a. 0.02
b. 0.15 *maddy -- I said this!, eva agrees*
c. 0.50
d. 0.55
...
d. They are the same dataset, but the first is not in the log scale, the second undersmooths, and the third oversmooths.*maddy -- I think this one!*
...
How many measurements do we have for each?
   *238 female measurements and 812 male measurements*
...
```{r}
#maddy answer
df <- data.frame("female_percentiles" = quantile(female, probs = c(0.1,0.3,0.5,0.7,0.9)) , "male_percentiles" = quantile(male, probs = c(0.1,0.3,0.5,0.7,0.9)))
```
...
Which continent has the country with the biggest population size? *Asia*
...
*maddy -- I think this looks like the Americas?*
...
#maddy answer
mean(x<=72) - mean(x<=69)
#so 0.334 of the data is between these two values
```
...
#approximated answer
approx <- pnorm(81,mu,sd)-pnorm(79,mu,sd)
...
```{r}
#Maddy answer
#inches in 7 feet is 7x12 = 84
seven_footers <- 1-pnorm(84, 69, 3)
#answer is 2.87e-7; very small!
```
...
```{r}
#Maddy answer
#inches in 6ft8 = 6*12 + 8  = 80 inches
lebrons <- 1-pnorm(80, 69, 3)
total_lebrons <- lebrons * 1e9
150/total_lebrons
...
c. As seen in question 10, the normal approximation tends to underestimate the extreme values. It's possible that there are more seven footers than we predicted.*Maddy -- I think this one!*
