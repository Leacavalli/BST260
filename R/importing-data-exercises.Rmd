...
#maddy answer
CE <- read_csv(file.path(path, "carbon_emissions.csv" ))
fert <- read_csv(file.path(path, "fertility-two-countries-example.csv" ))
le <- read_csv(file.path(path, "life-expectancy-and-fertility-two-countries-example.csv"))
murders <- read_csv(file.path(path, "murders.csv" ))
ssa <- read_csv(file.path(path, "ssa-death-probability.csv"  ))
olive <- read_csv(file.path(path, "olive.csv" ))
```
...
```{r}
#maddy answers
dat <- read_csv(file.path(path, "olive.csv" ), skip=1) #this skips all headers
dat2 <- read_csv(file.path(path, "olive.csv" ), name_repair = "minimal")
```
...
```{r}
#maddy answer
a <- readLines(con <- file(file.path(path, "olive.csv" )), n=1)
a
```
...
library(readxl)
filename <- "fake.xlsx"
path <- "/Users/madeleinekline/Dropbox (Harvard University)/G1/bst260"
fullpath <- file.path(path, filename)
file.copy(fullpath, filename)
d <- read_xlsx("fake.xlsx")
plot(hist(d$Time, breaks=1))
```
