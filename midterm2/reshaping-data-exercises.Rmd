...
co2_tidy <- co2_wide |> pivot_longer(!year,
                                     names_to = "month",
                                     values_to = "co2", names_transform = list(month=as.integer))
...
```{r}
#gender (men and women will be the columns in the tidy dataset)
...
tmp <-admissions |> pivot_longer(names_to = "name",
                                 values_to = "value",
...
```{r}
#minor code edit so that the column names read exactly as asked for in the question
...
```{r}
#names_from argument tells pivot_wider which variable will be used as the column names. The values_from argument specifies which variable to use to fill out the cells.
...
# another way to code this
admissions |>
  pivot_longer(names_to = "name",                                       #Q5
                           values_to = "value",
                          cols = admitted:applicants) |>
  unite(col = column_name, c(name,gender)) |>                           #Q6
  pivot_wider(names_from = column_name, values_from = value)            #Q7
