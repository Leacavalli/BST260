...
y_test<-as.numeric(as.character(dat$test$y))
sqrt(mean((y_hat - y_test)^2)) # Calculate the RMSE to find the accuracy of linear regression
#RMSE=1.053807

# Logistic model
fit_glm <- dat$train %>% glm(y ~ x, family = "binomial", data = .)
y_hat_glm <- as.numeric(as.character(ifelse(predict(fit_glm, dat$test) > 0.5, 1, 0) %>% factor(levels = c(0, 1))))
y_test<-as.numeric(as.character(dat$test$y))
mean(y_hat_glm == dat$test$y) # Prediction accuracy of the model
